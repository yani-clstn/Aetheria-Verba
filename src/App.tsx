...
function JournalHome({ 
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 md:px-12 py-4 md:py-8 max-w-7xl mx-auto relative overflow-x-hidden">
      {/* HEADER */}
      <header className="border-b border-aetheria-blackberry/20 dark:border-aetheria-beige/20 pb-4 md:pb-6 mb-6 md:mb-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex flex-col text-left">
            ...
            <Link to="/">
              <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-aetheria-blackberry dark:text-aetheria-beige hover:text-aetheria-teal transition-colors break-words">
                AETHERIA VERBA
              </h1>
            </Link>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto flex-shrink-0">
            {isSearchOpen ? (
              <div className="flex items-center bg-white/20 dark:bg-aetheria-cardDark border border-aetheria-teal/50 rounded-full px-3 py-1.5 shadow-[0_0_12px_rgba(45,212,191,0.2)]">
                <Search size={14} className="text-aetheria-teal mr-2 flex-shrink-0" />
                <input
                  type="text"
                  placeholder="Search..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="bg-transparent text-xs text-aetheria-blackberry dark:text-aetheria-beige outline-none w-28 sm:w-36 md:w-48 placeholder:text-aetheria-blackberry/50 dark:placeholder:text-aetheria-beige/50"
                  autoFocus
                />
                <button onClick={() => { setIsSearchOpen(false); setSearchQuery(''); }}>
                  <X size={14} className="flex-shrink-0 text-aetheria-blackberry dark:text-aetheria-beige" />
                </button>
              </div>
            ) : (
              <button
                onClick={() => setIsSearchOpen(true)}
                className="p-2 rounded-full border border-aetheria-blackberry/20 dark:border-aetheria-beige/20 hover:border-aetheria-teal hover:shadow-[0_0_15px_rgba(45,212,191,0.3)] hover:text-aetheria-teal transition-all text-aetheria-blackberry dark:text-aetheria-beige"
              >
                <Search size={18} />
              </button>
            )}
            ...
            <button
              ...
              className="p-2 rounded-full border border-aetheria-blackberry/20 dark:border-aetheria-beige/20 hover:border-aetheria-teal hover:shadow-[0_0_15px_rgba(45,212,191,0.3)] hover:text-aetheria-teal transition-all"
            >
              {darkMode ? (
                <Sun size={18} className="text-aetheria-beige hover:text-aetheria-teal transition-colors" />
              ) : (
                <Moon size={18} className="text-aetheria-blackberry hover:text-aetheria-teal transition-colors" />
              )}
            </button>
          </div>
        </div>
        ...
        {/* CATEGORY PILLS */}
        <div className="flex items-center justify-start gap-2 sm:gap-3 pt-4 sm:pt-6 overflow-x-auto pb-1 no-scrollbar">
          {['All', 'Featured Stories', 'Other Stories', 'Saved'].map((filterName) => (
            <button
              key={filterName}
              onClick={() => setSelectedFilter(filterName)}
              className={`px-3.5 py-1.5 text-xs rounded-full transition-all flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 ${
            ...
            >
              {featuredArticles.map((slide) => (
                <div key={slide.id} className="min-w-full flex-shrink-0 p-4 sm:p-6 md:p-8">
                  <Link to={`/article/${slide.slug}`} className="flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-6 items-stretch">
                    <div className="md:col-span-5 h-48 sm:h-64 md:h-80 overflow-hidden rounded-xl group/img relative w-full">
                      <img 
                        src={slide.imageUrl} 
                        alt={slide.title} 
                    ...
                    <div className="md:col-span-7 flex flex-col justify-between py-1">
                      <div>
                        <span className="inline-flex items-center gap-1 text-[10px] md:text-xs font-bold uppercase tracking-wider text-aetheria-teal mb-2">
                          <Sparkles size={14} className="animate-pulse" /> Highlight
                        </span>
                        <h2 className="font-serif text-xl sm:text-2xl md:text-4xl font-bold mb-2 md:mb-3 text-aetheria-blackberry dark:text-aetheria-beige hover:text-aetheria-teal transition-colors leading-snug">
                          {slide.title}
                        </h2>
                        <p className="text-xs sm:text-sm text-aetheria-blackberry/80 dark:text-aetheria-beige/80 line-clamp-3 mb-4 md:mb-6 leading-relaxed">
                          {slide.excerpt}
                        </p>
                      </div>
                      <div className="flex items-center justify-between text-xs text-aetheria-blackberry/60 dark:text-aetheria-beige/60 pt-2 border-t border-aetheria-blackberry/10 dark:border-aetheria-beige/10 mt-auto">
                        <span>{slide.date} • {slide.readTime}</span>
                        <span className="px-2.5 py-0.5 rounded-full border border-aetheria-teal text-aetheria-teal text-[10px] font-semibold shadow-[0_0_8px_rgba(45,212,191,0.2)]">
                          {slide.category}
                        </span>
                      </div>
                    </div>
            ...
            {featuredArticles.length > 1 && (
              <>
                <button 
                  ...
                  className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/60 text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all z-10"
                >
                  <ChevronLeft size={18} />
                </button>
                <button 
                  onClick={() => setCurrentSlide((prev) => (prev + 1) % featuredArticles.length)}
                  className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/60 text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all z-10"
                >
                  ...
                  {featuredArticles.map((_, idx) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all duration-300 ${
                        currentSlide === idx 
                          ? 'w-6 bg-aetheria-teal shadow-[0_0_10px_rgba(45,212,191,0.8)]' 
                ...
                <Link to={`/article/${article.slug}`} className="block flex-1 flex flex-col justify-between">
                  <div className="h-44 sm:h-48 overflow-hidden relative">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-2 rounded-full bg-black/60 backdrop-blur-md text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all z-10"
                    >
                  ...
                  <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="font-serif text-lg sm:text-xl font-bold mb-2 text-aetheria-blackberry dark:text-aetheria-beige group-hover:text-aetheria-teal transition-colors">
                        {article.title}
                      </h3>
                      <p className="text-xs sm:text-sm text-aetheria-blackberry/80 dark:text-aetheria-beige/80 line-clamp-2 mb-4">
                        {article.excerpt}
                      </p>
                    </div>
