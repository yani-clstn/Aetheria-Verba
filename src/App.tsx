...
function JournalHome({ 
  ...
  return (
    <div className="min-h-screen px-4 md:px-12 py-6 max-w-7xl mx-auto relative">
      {/* HEADER */}
      <header className="border-b border-aetheria-blackberry/20 dark:border-aetheria-beige/20 pb-6 mb-10">
        <div className="flex justify-between items-start">
          <div className="flex flex-col text-left">
            ...
            <Link to="/">
              <h1 className="font-serif text-3xl md:text-5xl font-bold tracking-tight text-aetheria-blackberry dark:text-aetheria-beige hover:text-aetheria-teal transition-colors">
                AETHERIA VERBA
              </h1>
            </Link>
          ...
          <div className="flex items-center gap-2 pt-1">
            {isSearchOpen ? (
              <div className="flex items-center bg-white/20 dark:bg-aetheria-cardDark border border-aetheria-teal/50 rounded-full px-3 py-1 shadow-[0_0_12px_rgba(45,212,191,0.2)]">
              ...
              {featuredArticles.map((slide) => (
                <div key={slide.id} className="min-w-full flex-shrink-0 p-6 md:p-8">
                  <Link to={`/article/${slide.slug}`} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                    ...
                    <div className="md:col-span-5 h-56 md:h-64 overflow-hidden rounded-xl group/img relative">
                      <img 
                        src={slide.imageUrl} 
                        alt={slide.title} 
            ...
            {featuredArticles.length > 1 && (
              <>
                <button 
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all opacity-0 group-hover:opacity-100 z-10 hover:shadow-[0_0_15px_rgba(45,212,191,0.5)]"
                >
                  <ChevronLeft size={18} />
                </button>
                <button 
                  onClick={() => setCurrentSlide((prev) => (prev + 1) % featuredArticles.length)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all opacity-0 group-hover:opacity-100 z-10 hover:shadow-[0_0_15px_rgba(45,212,191,0.5)]"
                >
                ...
                <Link to={`/article/${article.slug}`} className="block flex-1 flex flex-col justify-between">
                  <div className="h-48 overflow-hidden relative">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-2 rounded-full bg-black/50 backdrop-blur-md text-white hover:bg-aetheria-teal hover:text-aetheria-dark transition-all z-10 hover:shadow-[0_0_12px_rgba(45,212,191,0.5)]"
                    >
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <h4 className="font-serif text-xl font-bold mb-2 text-aetheria-blackberry dark:text-aetheria-beige group-hover:text-aetheria-teal transition-colors">
                        {article.title}
                      </h4>
...
function ArticleDetailPage() {
 ...
 useEffect(() => {
  ...
  return (
    <div className="min-h-screen px-4 md:px-12 py-8 max-w-4xl mx-auto text-aetheria-blackberry dark:text-aetheria-beige">
      ...
      <article>
        ...
        <h1 className="font-serif text-4xl md:text-6xl font-bold leading-tight mb-4">{article.title}</h1>
        <p className="text-base md:text-lg opacity-80 mb-6 italic font-serif">{article.excerpt}</p>
        <img src={article.imageUrl} alt={article.title} className="w-full h-80 md:h-[450px] object-cover rounded-2xl mb-8" />
        <div className="prose dark:prose-invert max-w-2xl mx-auto font-serif text-lg leading-relaxed space-y-6">
